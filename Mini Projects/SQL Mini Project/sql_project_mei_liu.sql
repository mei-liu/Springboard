...
Springboard' online SQL platform. The data you need is in the "country_club" database. This database
contains 3 tables:
...
SELECT b.name AS facility, 
	CONCAT(c.firstname, " ", c.surname) AS member,
	CASE WHEN  c.surname = 'GUEST'
		THEN  b.guestcost * a.slots
		ELSE  b.membercost * a.slots
    	END AS cost
FROM Bookings a
INNER JOIN Facilities b ON a.facid = b.facid
INNER JOIN Members c ON a.memid = c.memid
WHERE a.starttime BETWEEN '2012-09-14' AND '2012-09-15'
HAVING cost > 30
ORDER BY cost DESC
